...
| 8 | What are the advantages and disadvantages of RLM versus other reasoning paradigms? | Порівняння |
...
| 8 | Пере/недоліки RLM | \`rlm_rl_training_alphaxiv_chunk_050\` | 0.6584 | rlm_rl_training_alphaxiv |
...
| Метрика | Значення |
|---------|----------|
| Середня оцінка | 0.7152 |
| Мінімальна | 0.6229 |
| Максимальна | 0.8503 |
| Запитів з оцінкою > 0.60 | 10/10 |
...
| 8 | Пере/недоліки RLM | 0.6584 | rlm_rl_training_alphaxiv | ✅ Влучив | Знайшов чанк про переваги/недоліки RLM порівняно з іншими парадигмами |
...
|---------|----------|
| Середня оцінка | **0.7152** |
| Мінімальна | 0.6229 (Q10: RL-донавчання) |
| Максимальна | 0.8503 (Q2: контекстна деградація) |
| Запитів > 0.65 (сильні) | **8/10** |
| Запитів > 0.60 (прийнятні) | **10/10** |
| Запитів < 0.60 (слабкі) | **0/10** |
...
1. **Порівняльні запити** — Q6 (0.64) та Q10 (0.62) мають оцінки <0.65. Запити типу "X проти Y" втрачають семантичну специфічність.
2. **Запити про еволюцію концепцій** — Q7 (0.67) на межі. Запити про еволюцію парадигм складні для чистого семантичного пошуку.
3. **Закономірність:** нижчі оцінки корелюють із запитами, що містять порівняння або абстракцію.

### 🎯 Висновок

Семантичний пошук на MiniLM + FAISS показує стабільну базову продуктивність (середній показник 0.72, 10/10 >0.60). Найкраще працює для концептуальних запитів та запитів з унікальними термінами. Найслабші місця — порівняльні запити, що втрачають семантичну специфічність. Ці обмеження є очікуваними для чистого семантичного пошуку.
